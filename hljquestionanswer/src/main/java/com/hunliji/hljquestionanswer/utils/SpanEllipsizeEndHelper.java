...
public class SpanEllipsizeEndHelper {
    ...
    private static final LruCache<String, SpannableStringBuilder> SPAN_MAXWIDTH_CACHE = new LruCache<>(SPANNABLE_MAXWIDTH_CACHE_SIZE);
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    public static CharSequence matchMaxWidth(SpannableStringBuilder targetText, TextView textView) {
        if(targetText==null){
            return null;
        }
        ...
        SpannableStringBuilder tmpText = SPAN_MAXWIDTH_CACHE.get(maxWidthKey);
        if (tmpText != null) {
...
//            Log.d(TAG, String.format("targetX: %d, currentX: %d, currentLength: %d, totalLength: %d, emojiStrW[%d], emojiDraW[%d]", targetX, x, middle, targetText.length(), emojiStrW, emojiDraW));
        ...
        tmpText = (SpannableStringBuilder) tmpText.subSequence(0, middle + 1);
//        Log.d(TAG, String.format("sub Sequence[0, %d), [%s] to [%s]", middle + 1, targetText, tmpText));

        // add ...
        final SpannableStringBuilder maxWidthSS = new SpannableStringBuilder(tmpText + "...");
