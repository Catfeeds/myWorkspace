...
public class CustomTextView extends TextView {
    ...
    private float getTextViewLength(String text) {
        float textLength = testPaint.measureText(text);
        return textLength;
    }

    public void setImageSpanText(String text, int spanWidth,
                                 int verticalAlignment, int maxWidth) {
        ...
        if (JSONUtil.isEmpty(textString) || !textString.equals(text)) {
    ...
    private void setNewText() {
        ...
        while (matcher.find()) {
            ...
            textWidth += getTextViewLength(textString.substring(last,
                    matcher.start()));
            ...
            if (resId != 0) {
                ...
                textWidth += getTextViewLength(textString.substring(
                        matcher.start(), last));
                builder.append(tag);
            }
        }
