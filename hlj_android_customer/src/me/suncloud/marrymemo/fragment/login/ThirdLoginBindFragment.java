...
public class ThirdLoginBindFragment extends RefreshFragment {
    ...
    public void onGetCertify(View view) {
                ...
                .setOnNextListener(new SubscriberOnNextListener<HljHttpResult<CertifyCodeMsg>>() {
                    ...
                    public void onNext(HljHttpResult<CertifyCodeMsg> result) {
                        ...
                        if (status != null && status.getRetCode() != 0) {
                            if (status.getRetCode() != LoginHelper.CERTIFY_CODE) {
        ...
        if (view.getId() == R.id.btn_voice_certify) {
            ...
            certifySubscription = LoginApi.getPostCertifyCode(new CertifyPostBody(phone,
                    LoginHelper.BINDPHONEVOICE))
                    .subscribe(subscriber);
        } else {
            certifySubscription = LoginApi.getPostCertifyCode(new CertifyPostBody(phone,
                    LoginHelper.BINDPHONE))
                    .subscribe(subscriber);
        }
    }
    ...
    public void onCertifyCheck() {
        ...
        Session.getInstance()
                .clearLogout(getContext());
        ThirdLoginPostBody body = new ThirdLoginPostBody();
        body.setOpenid(parameter.getThirdId());
        body.setPhone(phone);
        body.setSms_code(code);
        body.setUser_info(parameter.getLoginInfo());
        body.setType(parameter.getType());
        loginSubscription = LoginApi.thirdRegister(body)
                        ...
                        .setOnNextListener(new SubscriberOnNextListener<LoginResult>() {
                            ...
                            public void onNext(LoginResult loginResult) {
                                if (getActivity() != null && getActivity() instanceof
                                        PhoneLoginFragment.LoginCallback) {
                                    ((PhoneLoginFragment.LoginCallback) getActivity()).onComplete(
                                            loginResult);
                                    User user = Session.getInstance()
                                            .getCurrentUser(getContext());
                                    if (user != null && user.getId() > 0) {
                                        new HljTracker.Builder(getContext()).eventableId(user
                                                .getId())
                                                .eventableType("User")
                                                .screen("login")
                                                .action("login")
                                                .additional(parameter.getType())
                                                .build()
                                                .add();
                                    }
                                }
