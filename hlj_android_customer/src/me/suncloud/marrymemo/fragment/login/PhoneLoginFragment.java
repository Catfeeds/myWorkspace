    ...
    public void onGetCertify(View view) {
        ...
        if (view.getId() == R.id.btn_voice_certify) {
            ...
            certifySubscription = LoginApi.getPostCertifyCode(new CertifyPostBody(phone,
                    LoginHelper.LOGINVOICE))
                    .subscribe(subscriber);
        } else {
            certifySubscription = LoginApi.getPostCertifyCode(new CertifyPostBody(phone,
                    LoginHelper.LOGINMSG))
                    .subscribe(subscriber);
        }
    }
                        ...
                        .setOnNextListener(new SubscriberOnNextListener<LoginResult>() {
                            ...
                            public void onNext(LoginResult loginResult) {
                                if (getActivity() != null && getActivity() instanceof
                                        LoginCallback) {
                                    ((LoginCallback) getActivity()).onComplete(loginResult);
                                }
                            }
                        })
                        .build());
                        ...
                        .setOnNextListener(new SubscriberOnNextListener<HljHttpResult<LoginResult>>() {
                            ...
                            public void onNext(HljHttpResult<LoginResult> result) {
                                if (getActivity() != null && getActivity() instanceof
                                    ...
                                    if (result.getStatus()
                                            .getRetCode() == 0) {
                                        ((LoginCallback) getActivity()).onComplete(result.getData
                                                ());
                                        ...
                                        if (user != null && user.getId() > 0) {
                                            new HljTracker.Builder(getContext()).eventableId(user
                                                    ...
                                                    .screen("login")
                                                    .action("login")
                                                    .additional(loginParameter.getType())
                                                    .build()
                                                    .add();
                                        }
                                    } else if (result.getStatus()
                                            .getRetCode() == 404) {
                                        ((LoginCallback) getActivity()).onThirdBind(loginParameter);
                                    }
                                }
                            }
                        })
