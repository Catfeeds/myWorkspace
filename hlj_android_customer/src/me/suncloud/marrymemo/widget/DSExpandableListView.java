 ...
 * @author heycosmo
 */
public class DSExpandableListView extends SwipeExpandableListView {
    ...
    public DSExpandableListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            ...
            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(R.styleable
                    .DragSortListView_collapsed_height, 1));
            ...
            boolean useDefault = a.getBoolean(R.styleable
                    .DragSortListView_use_default_controller, true);
            ...
            if (useDefault) {
                ...
                int flingHandleId = a.getResourceId(R.styleable.DragSortListView_fling_handle_id,
                        0);
                int clickRemoveId = a.getResourceId(R.styleable.DragSortListView_click_remove_id,
                        0);
                int bgColor = a.getColor(R.styleable.DragSortListView_float_background_color,
                        Color.BLACK);

                DSExpandableController controller = new DSExpandableController(this,
                        dragHandleId, dragInitMode, removeMode, clickRemoveId, flingHandleId);
    ...
    private boolean updatePositions() {
        ...
        if (mFirstExpPos != oldFirstExpPos || mSecondExpPos != oldSecondExpPos || mSlideFrac !=
                oldSlideFrac) {
            updated = true;
        }
    ...
    private void continueDrag(int x, int y) {
        ...
        } else if (maxY >= mUpScrollStartY && minY <= mDownScrollStartY && mDragScroller
                .isScrolling()) {
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        if (position == mFirstExpPos || position == mSecondExpPos) {
            if (position < mSrcPos) {
                ((DSSwipableItemView) v).setGravity(Gravity.BOTTOM);
            } else if (position > mSrcPos) {
                ((DSSwipableItemView) v).setGravity(Gravity.TOP);
            }
        }
     ...
     *
     * @param position  SearchItem to drag.
     ...
     * {@link me.suncloud.marrymemo.widget.DSExpandableListView.DragScrollProfile}.
     *
     * @param ssp
     */
     ...
     * your own is not passed, the default {@link com.mobeta.android.dslv.SimpleFloatViewManager}
    ...
    public interface FloatViewManager {
         ...
         *                 want to call {@link android.widget.ListView#getChildAt(int)},
         *                 you will need
         *                 to add {@link android.widget.ListView#getHeaderViewsCount()} to the
         *                 index).
         ...
         *                  and the offset provided in {@link me.suncloud.marrymemo.widget
         *                  .DragSortExpandableListView#startDrag}.
     ...
     * {@link me.suncloud.marrymemo.widget.DSExpandableListView#setDragScrollProfile
     * (DragScrollProfile)} to
     * set custom profile.
     *
     * @author heycosmo
     */
    ...
    /**
     * 设置额外新增view的宽
     * @param mAddedViewWidth
     */
    public void setmAddedViewWidth(int mAddedViewWidth) {
        this.mAddedViewWidth = mAddedViewWidth;
    }
    ...
    private class AdapterWrapper extends BaseExpandableListAdapter {
        ...
        public View getGroupView(int groupPosition, boolean isExpanded, View convertView,
                                 ViewGroup parent) {
            DSSwipableItemView v;
            View child;
            // Log.d("mobeta",
            // "getView: position="+position+" convertView="+convertView);
            if (convertView != null) {
                v = (DSSwipableItemView) convertView;
                View oldChild = v.getChildAt(0);

                child = mAdapter.getGroupView(groupPosition, isExpanded, oldChild,
                        DSExpandableListView.this);
                ...
                child = mAdapter.getGroupView(groupPosition, isExpanded, null,
                        DSExpandableListView.this);

                    v = new DSSwipableItemView(getContext());
            ...
            v.setmAddedViewWidth(mAddedViewWidth);
        ...
        public View getChildView(int groupPosition, int childPosition, boolean isLastChild,
            ...
            return mAdapter.getChildView(groupPosition, childPosition, isLastChild, convertView,
                    parent);
        }
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                DSExpandableListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            DSExpandableListView.this.layoutChildren();
            invalidate();
    ...
    private class DragSortTracker {
        ...
        public void appendState() {
            ...
            mBuilder.append("    <FirstExpBlankHeight>").append(getItemHeight(mFirstExpPos) -
                    getChildHeight(mFirstExpPos)).append("</FirstExpBlankHeight>\n");
            mBuilder.append("    <SecondExpPos>").append(mSecondExpPos).append("</SecondExpPos>\n");
            mBuilder.append("    <SecondExpBlankHeight>").append(getItemHeight(mSecondExpPos) -
                    getChildHeight(mSecondExpPos)).append("</SecondExpBlankHeight>\n");
