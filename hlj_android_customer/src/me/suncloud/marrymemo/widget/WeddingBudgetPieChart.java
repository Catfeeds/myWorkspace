 ...
 */
public class WeddingBudgetPieChart extends PieRadarChartBase<PieData> {
    ...
    public WeddingBudgetPieChart(Context context) {
        super(context);
    }

    public WeddingBudgetPieChart(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public WeddingBudgetPieChart(Context context, AttributeSet attrs, int
            defStyle) {
    ...
    protected void init() {
        super.init();
        mContext = getContext();
        mRenderer = new WeddingBudgetPieChartRenderer(getContext(), this,
                mAnimator, mViewPortHandler);
        DecimalFormat format = new DecimalFormat("0.0%");
        format.setRoundingMode(RoundingMode.HALF_UP);
        ((WeddingBudgetPieChartRenderer)mRenderer).setFormat(format);
        mXAxis = null;
    }

    public void setOutOffset(int outOffset) {
        ((WeddingBudgetPieChartRenderer) mRenderer).setOffset(outOffset);
        invalidate();
    }
    ...
    public float getSlice() {
        return mData.getDataSet().getSliceSpace();
    }
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        float x = (float) (r
                * Math.cos(Math.toRadians((rotationAngle + mAbsoluteAngles[i]
                - offset)
                * mAnimator.getPhaseY())) + center.x);
        float y = (float) (r
                * Math.sin(Math.toRadians((rotationAngle + mAbsoluteAngles[i]
                - offset)
    ...
    private void calcAngles() {
        ...
        for (int i = 0; i < mData.getDataSetCount(); i++) {
            ...
            for (int j = 0; j < set.getEntryCount(); j++) {
                ...
                mDrawAngles[cnt] = calcAngle(Math.abs(set.getEntryForIndex(j)
                        .getVal()), yValueSum);
    ...
    public void setHoleColor(int color) {
        ((WeddingBudgetPieChartRenderer) mRenderer).getPaintHole().setColor
                (color);
    }

    /**
     * Enable or disable the visibility of the inner tips of the slices
     * behind the hole
     */
    ...
    public void setCenterTextTypeface(Typeface t) {
        ((WeddingBudgetPieChartRenderer) mRenderer).getPaintCenterText()
                .setTypeface(t);
    }
    ...
    public void setTransparentCircleColor(int color) {
        ...
        Paint p = ((WeddingBudgetPieChartRenderer) mRenderer)
    ...
    public void setTransparentCircleAlpha(int alpha) {
        ((WeddingBudgetPieChartRenderer) mRenderer).getPaintTransparentCircle
                ().setAlpha
                (alpha);
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mRenderer != null && mRenderer instanceof
                WeddingBudgetPieChartRenderer) {
            ((WeddingBudgetPieChartRenderer) mRenderer).releaseBitmap();
        }
