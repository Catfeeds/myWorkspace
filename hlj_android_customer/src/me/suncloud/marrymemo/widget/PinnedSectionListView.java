...
/**
 * ListView, which is capable to pin section views at its top while the rest
 * is still scrolled.
 */
public class PinnedSectionListView extends ListView {
    ...
    private final OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScroll(
            ...
            if (isFirstVisibleItemSection) {
                ...
                if (sectionView.getTop() == getPaddingTop()) { // view sticks
                    // to the top, no need for pinned shadow
                    destroyPinnedShadow();
                } else { // section doesn't stick to the top, make sure we
                    // have a pinned shadow
                ...
                int sectionPosition = findCurrentSectionPosition(
                        firstVisibleItem);
        ...
        if (visible) {
            if (mShadowDrawable == null) {
                mShadowDrawable = new GradientDrawable(Orientation.TOP_BOTTOM,
                        new int[]{Color.parseColor("#ffa0a0a0"), Color
                                .parseColor(
                                "#50a0a0a0"), Color.parseColor("#00a0a0a0")});
                mShadowHeight = (int) (8 * getResources().getDisplayMetrics()
                        .density);
            }
        } else {
            if (mShadowDrawable != null) {
    ...
    void createPinnedShadow(int position) {
        ...
        if(onPinnedViewListener != null){
            onPinnedViewListener.onPinnedView(pinnedView);
        }
        // read layout parameters
        ViewGroup.LayoutParams layoutParams = (ViewGroup.LayoutParams)
                pinnedView.getLayoutParams();
    ...
    private OnPinnedViewListener onPinnedViewListener;

    public void setOnPinnedViewListener(OnPinnedViewListener onPinnedViewListener){
        this.onPinnedViewListener = onPinnedViewListener;
    }
    /**
     * item view listener
     */
    public interface OnPinnedViewListener{
        void onPinnedView(View pinnedView);
    }
    ...
    void ensureShadowForPosition(
        ...
        if (mPinnedSection != null && mPinnedSection.position !=
                sectionPosition) { // invalidate shadow, if required
            destroyPinnedShadow();
        }
    ...
    int findFirstVisibleSectionPosition(
        ...
        if (firstVisibleItem + visibleItemCount >= adapterDataCount) {//added
            // to prevent index Outofbound (in case)
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (adapter != null) {
            if (!(adapter instanceof PinnedSectionListAdapter))
                throw new IllegalArgumentException(
                        "Does your adapter implement " +
                                "PinnedSectionListAdapter?");
            if (adapter.getViewTypeCount() < 2)
                throw new IllegalArgumentException(
                        "Does your adapter handle at least two types" + " of " +
                                "views in getViewTypeCount() method: items " +
                                "and sections?");
        }
        ...
        // onDestroy pinned shadow, if new adapter is not same as old one
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN && mTouchTarget == null &&
                mPinnedSection != null && isPinnedViewTouched(mPinnedSection
                .view,
                x,
                y)) { // create touch target
        ...
        if (mTouchTarget != null) {
            ...
            if (action == MotionEvent.ACTION_UP) { // perform onClick on
                // pinned view
    ...
    private boolean performPinnedItemClick() {
        ...
        if (listener != null && getAdapter().isEnabled(mPinnedSection
                .position)) {
            View view = mPinnedSection.view;
            playSoundEffect(SoundEffectConstants.CLICK);
            if (view != null) {
                view.sendAccessibilityEvent(AccessibilityEvent
                        .TYPE_VIEW_CLICKED);
            }
    ...
    public static boolean isItemViewTypePinned(
        ...
        return ((PinnedSectionListAdapter) adapter).isItemViewTypePinned(
                viewType);
    }
