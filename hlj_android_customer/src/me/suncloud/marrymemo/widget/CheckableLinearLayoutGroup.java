...
public class CheckableLinearLayoutGroup extends LinearLayout {
    ...
    public void addView(@NonNull View child, int index, ViewGroup.LayoutParams params) {
        if (child instanceof CheckableLinearLayoutButton) {
            ...
            // 如果没有设置id,则随机生成一个与现有不重复的
            if (button.getId() == -1) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    button.setId(View.generateViewId());
                }else {
                    button.setId(Util.getUniqueId(childIds));
                }
            }

            childIds.add(button.getId());
    ...
    private class CheckedStateTracker implements CheckableLinearLayoutButton
    ...
    private class PassThroughHierarchyChangeListener implements ViewGroup
            .OnHierarchyChangeListener {
        private ViewGroup.OnHierarchyChangeListener mOnHierarchyChangeListener;

        @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
        public void onChildViewAdded(View parent, View child) {
            if (parent == CheckableLinearLayoutGroup.this && child instanceof
                    CheckableLinearLayoutButton) {
                int id = child.getId();
