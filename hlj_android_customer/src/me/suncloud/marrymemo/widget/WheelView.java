...
public class WheelView extends View {
    ...
    Typeface typeface = Typeface.DEFAULT;//字体样式，默认是等宽字体
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textSize = getResources().getDimensionPixelSize(R.dimen.large_text_size);//默认大小
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.hljpickerview, 0, 0);
            mGravity = a.getInt(R.styleable.hljpickerview_hljpickerview_gravity, Gravity.CENTER);
            textColorOut = a.getColor(R.styleable.hljpickerview_hljpickerview_textColorOut,
                    textColorOut);
            textColorCenter = a.getColor(R.styleable.hljpickerview_hljpickerview_textColorCenter,
                    textColorCenter);
            dividerColor = a.getColor(R.styleable.hljpickerview_hljpickerview_dividerColor,
                    dividerColor);
            textSize = a.getDimensionPixelOffset(R.styleable.hljpickerview_hljpickerview_textSize,
                    textSize);
            lineSpacingMultiplier = a.getFloat(R.styleable
                            .hljpickerview_hljpickerview_lineSpacingMultiplier,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                //将文字进行截取
                float measureTextWidth = paintCenterText.measureText(contentText);
                int screenSize = CommonUtil.getDeviceSize(getContext()).x;
                float scale = measureTextWidth / screenSize;
                if (scale > 0.6) {
                    contentText = contentText.substring(0,
                            (int) (contentText.length() / scale * 0.6)) + "...";
                }
