...
public class TextCountWatcher implements TextWatcher {
    ...
    public void afterTextChanged(Editable s) {
        ...
        int outCount = CommonUtil.getTextLength(s) - limitCount;
        while (outCount > 0) {
            editStart -= outCount;
            s.delete(editStart, editEnd);
            editEnd = editStart;
            outCount = CommonUtil.getTextLength(s) - limitCount;
        }
        if (numText != null) {
            numText.setText(String.valueOf(limitCount - CommonUtil.getTextLength(s)));
        }
