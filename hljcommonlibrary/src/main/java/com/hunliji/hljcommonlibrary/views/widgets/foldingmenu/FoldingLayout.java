package com.hunliji.hljcommonlibrary.views.widgets.foldingmenu;
...
public class FoldingLayout extends ViewGroup {
    ...
    private final float SHADING_ALPHA = 0f;
    private final float SHADING_FACTOR = 0f;
    private final int DEPTH_CONSTANT = 1200;
    ...
    private float mFoldFactor = 0; //默认展开
    ...
    private void prepareFold(
            Orientation orientation, float anchorFactor, int numberOfFolds) {
        ...
        if (mIsHorizontal) {
            mShadowLinearGradient = new LinearGradient(0,
                    0,
                    SHADING_FACTOR,
                    0,
                    Color.BLACK,
        ...
        int delta = Math.round(mIsHorizontal ? ((float) w) / ((float) mNumberOfFolds) : ((float)
                h) / ((float) mNumberOfFolds));
    ...
    private void calculateMatrices() {
        ...
        float depth = mIsHorizontal ? (float) Math.sqrt((double) (mFoldDrawWidth * mFoldDrawWidth
                - translatedDistanceFoldSquared)) : (float) Math.sqrt(
                (double) (mFoldDrawHeight * mFoldDrawHeight - translatedDistanceFoldSquared));
        ...
        float anchorPoint = mIsHorizontal ? mAnchorFactor * mOriginalWidth : mAnchorFactor *
                mOriginalHeight;
        ...
        for (int x = 0; x < mNumberOfFolds; x++) {
            ...
            if (mIsHorizontal) {
                ...
                mDst[4] = (anchorPoint > (x + 1) * mFoldDrawWidth) ? anchorPoint + (x + 1 -
                        midFold) * scaledWidth : anchorPoint - (midFold - x - 1) * scaledWidth;
