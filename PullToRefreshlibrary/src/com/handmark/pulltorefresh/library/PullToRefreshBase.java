...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements
    ...
    private AnimationStyle mStartLoadingAnimationStyle = AnimationStyle.getDefault();
    private AnimationStyle mEndLoadingAnimationStyle = AnimationStyle.getDefault();
    ...
    private boolean isNeedChangeSize = true;
    ...
    public PullToRefreshBase(Context context, Mode mode, AnimationStyle animStyle) {
        super(context);
        mMode = mode;
        mStartLoadingAnimationStyle = animStyle;
        mEndLoadingAnimationStyle = animStyle;
        init(context, null);
    }
    ...
    public final boolean isPullToRefreshOverScrollEnabled() {
        return VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD && mOverScrollEnabled &&
                OverscrollHelper.isAndroidOverScrollEnabled(
                mRefreshableView);
    }
    ...
    public final void setRefreshing(boolean doScroll) {
        if (!isRefreshing()&&mMode!=Mode.DISABLED) {
    ...
    public void setNeedChangeSize(boolean isNeedChangeSize) {
        this.isNeedChangeSize = isNeedChangeSize;
    }
    ...
    public void onRefreshComplete() {
        if (mHeaderLayout.isComplete()) {
            return;
        }
        mHeaderLayout.onRefreshComplete();
        if (!isRefreshing()) {
            return;
        }
    ...
    protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
        ...
        switch (mode) {
            case PULL_FROM_START:
                mLoadingAnimationStyle = mStartLoadingAnimationStyle;
                break;
            case PULL_FROM_END:
            default:
                mLoadingAnimationStyle = mStartLoadingAnimationStyle;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!isNeedChangeSize) {
            ...
            refreshRefreshableViewSize(widthMeasureSpec, heightMeasureSpec);
        }
    ...
    protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (isNeedChangeSize || !isLoadOnce) {
            if (DEBUG) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrStartAnimationStyle)) {
            mStartLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(R.styleable
                            .PullToRefresh_ptrStartAnimationStyle,
                    0));
        }
        if (a.hasValue(R.styleable.PullToRefresh_ptrEndAnimationStyle)) {
            mEndLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(R.styleable
                            .PullToRefresh_ptrEndAnimationStyle,
                    0));
        }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            case PULL_FROM_END:
                newScrollValue = Math.round(Math.max(initialMotionValue - lastMotionValue,
                        0) / FRICTION);
                itemDimension = getFooterSize();
                break;
            case PULL_FROM_START:
            default:
                newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue,
                        0) / FRICTION);
    ...
    public void setRefreshScrollDeltaChangedListener(
            OnRefreshScrollDeltaChangedListener changedListener) {
    ...
    public static interface OnPullEventListener<V extends View> {
        ...
        public void onPullEvent(
                final PullToRefreshBase<V> refreshView, State state, Mode direction);
    ...
    public void setHeaderColor(@ColorInt int headerColor) {
        ...
        if (mHeaderLayout != null) {
            mHeaderLayout.setBackgroundColor(headerColor);
        }
    }

    public void setFooterColor(@ColorInt int footerColor) {
        this.footerColor = footerColor;
        if (mFooterLayout != null) {
            mFooterLayout.setBackgroundColor(footerColor);
        }
    }
}
