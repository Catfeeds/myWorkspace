...
public class ProgressBarView extends View implements OnRangeSeekBarListener,
    ...
    private void init() {
        int lineProgress = ContextCompat.getColor(getContext(), R.color.colorPrimary);
        int lineBackground = ContextCompat.getColor(getContext(), R.color.colorBlack2);

        mProgressHeight = CommonUtil.dp2px(getContext(), 2);
    ...
    private void updateBackgroundRect(int index, float value) {
        ...
        if (index == 0) {
            mBackgroundRect = new Rect(newValue,
                    mBackgroundRect.top,
                    mBackgroundRect.right,
                    mBackgroundRect.bottom);
        } else {
            mBackgroundRect = new Rect(mBackgroundRect.left,
                    mBackgroundRect.top,
                    newValue,
                    mBackgroundRect.bottom);
        }
    ...
    public void updateProgress(int time, int max, float scale) {
        ...
        if (scale == 0) {
            ...
            mProgressRect = new Rect(mBackgroundRect.left,
                    mBackgroundRect.top,
                    newValue,
                    mBackgroundRect.bottom);
        }
