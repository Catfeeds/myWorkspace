 ...
 */
public class EasyVideoPlayer extends FrameLayout implements IUserMethods, TextureView
        .SurfaceTextureListener, MediaPlayer.OnPreparedListener, MediaPlayer
        .OnBufferingUpdateListener, MediaPlayer.OnCompletionListener, MediaPlayer
        .OnVideoSizeChangedListener, MediaPlayer.OnErrorListener, View.OnClickListener, SeekBar
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme()
                    .obtainStyledAttributes(attrs, R.styleable.EasyVideoPlayer, 0, 0);
            try {
                ...
                mLeftAction = a.getInteger(R.styleable.EasyVideoPlayer_evp_leftAction,
                        LEFT_ACTION_RESTART);
                //noinspection WrongConstant
                mRightAction = a.getInteger(R.styleable.EasyVideoPlayer_evp_rightAction,
                        RIGHT_ACTION_NONE);
                ...
                int restartDrawableResId = a.getResourceId(R.styleable
                                .EasyVideoPlayer_evp_restartDrawable,
                        -1);
                int playDrawableResId = a.getResourceId(R.styleable
                                .EasyVideoPlayer_evp_playDrawable,
                        -1);
                int pauseDrawableResId = a.getResourceId(R.styleable
                                .EasyVideoPlayer_evp_pauseDrawable,
                        -1);

                if (restartDrawableResId != -1) {
                    mRestartDrawable = AppCompatResources.getDrawable(context,
                            restartDrawableResId);
                }
                ...
                mHideControlsOnPlay = a.getBoolean(R.styleable
                                .EasyVideoPlayer_evp_hideControlsOnPlay,
                        true);
                mAutoPlay = a.getBoolean(R.styleable.EasyVideoPlayer_evp_autoPlay, false);
                mControlsDisabled = a.getBoolean(R.styleable.EasyVideoPlayer_evp_disableControls,
                        false);

                mThemeColor = a.getColor(R.styleable.EasyVideoPlayer_evp_themeColor,
                        Color.WHITE);

                mAutoFullscreen = a.getBoolean(R.styleable.EasyVideoPlayer_evp_autoFullscreen,
                        false);
        ...
        if (mRetryText == null)
            mRetryText = context.getResources()
                    .getText(R.string.evp_retry);
        if (mSubmitText == null)
            mSubmitText = context.getResources()
                    .getText(R.string.evp_submit);

        if (mRestartDrawable == null)
            mRestartDrawable = AppCompatResources.getDrawable(context,
                    R.drawable.evp_action_restart);
        if (mPlayDrawable == null)
            mPlayDrawable = AppCompatResources.getDrawable(context,
                    R.mipmap.icon_video_preview_play);
        if (mPauseDrawable == null)
            mPauseDrawable = AppCompatResources.getDrawable(context,
                    R.mipmap.icon_video_preview_pause);
    }
    ...
    private void setSourceInternal() throws IOException {
        if (mSource.getScheme() != null && (mSource.getScheme()
                .equals("http") || mSource.getScheme()
                .equals("https"))) {
            LOG("Loading web URI: " + mSource.toString());
            mPlayer.setDataSource(mSource.toString());
        } else if (mSource.getScheme() != null && (mSource.getScheme()
                .equals("file") && mSource.getPath()
                .contains("/android_assets/"))) {
            LOG("Loading assets URI: " + mSource.toString());
            AssetFileDescriptor afd;
            afd = getContext().getAssets()
                    .openFd(mSource.toString()
    ...
    protected void onFinishInflate() {
        ...
        textureLp.setMargins(0, 0, 0, CommonUtil.dp2px(getContext(), 60));
    ...
    private void tintSelector(@NonNull View view, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && view.getBackground()
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if (mAutoFullscreen) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    flags |= View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View
                            .SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION | View
                            .SYSTEM_UI_FLAG_LAYOUT_STABLE;
                    if (fullscreen) {
                        flags |= View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View
