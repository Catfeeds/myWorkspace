 ...
 */
public class WeddingCarCommentMarksViewHolder extends
        BaseViewHolder<List<CommentMark>> {
    @BindView(R2.id.marks_layout)
    LinearLayout marksLayout;
    @BindView(R2.id.marks_flow_layout)
    ...
    public WeddingCarCommentMarksViewHolder(View itemView) {
        super(itemView);
    ...
    protected void setViewData(
        ...
        for (int i = 0; i < size; i++) {
            ...
            final CommentMark mark = marks.get(i);
            if (count > i) {
                checkBox = (CheckBox) marksFlowLayout.getChildAt(i);
            }
            if (checkBox == null) {
                View.inflate(mContext,
                        R.layout.wedding_car_comment_marks_flow_item___car,
                        marksFlowLayout);
            ...
            if (mark.getId() == CommentMark.ID_BAD_REPUTATION) {
