...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        mRighted = a.getBoolean(R.styleable.CirclePageIndicator_arighted, false); // 获取靠右属性,默认不靠右
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f) - ((count *
                    threeRadius-mCircleOffset) / 2.0f);
        }

        // 重新计算如果靠右的 offset 值
        if (mRighted) {
            longOffset += (longSize - longPaddingBefore - longPaddingAfter) - (count * threeRadius-mCircleOffset);
        }
